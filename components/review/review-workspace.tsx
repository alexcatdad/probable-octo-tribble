...
export const ReviewWorkspace = memo(function ReviewWorkspace({
  matter,
  ...
  const firstFindingByClause = useMemo(
    () =>
      Object.fromEntries(
  ...
  const clearPreview = useCallback((source?: "queue" | "document") => {
  ...
  const handlePreviewClause = useCallback(
    (clauseId: string | null) => {
      if (!clauseId) {
    ...
    [clearPreview, firstFindingByClause],
  );

  const handlePreviewFinding = useCallback(
    (findingId: string | null, source: "queue" | "document") => {
      if (!findingId) {
        clearPreview(source);
        return;
      }
    ...
    [clearPreview, findingById],
  );

  const showUndoToast = useCallback(
    (findingId: string, actionType: string) => {
      const finding = findingById[findingId];
      setUndoState({
        findingId,
  ...
  const handleAcceptSuggestion = useCallback(
    ...
    [review.acceptSuggestion, showUndoToast],
  );
  ...
  const handleRejectSuggestion = useCallback(
    ...
    [review.rejectSuggestion, showUndoToast],
  );

  const handleMarkNeedsFollowUp = useCallback(
    (findingId: string) => {
      review.markNeedsFollowUp(findingId, reviewActionCopy.followUpNote);
      showUndoToast(findingId, "mark_needs_follow_up");
    },
    [review.markNeedsFollowUp, showUndoToast],
  );
  ...
  const handleUndo = useCallback(
    ...
    [review.revertDecision],
  );

  const handleSelectClauseFromDocument = useCallback(
    (clauseId: string) => {
      clearPreview();
      review.selectClause(clauseId);
    },
    [clearPreview, review.selectClause],
  );

  const handleSelectFindingFromRail = useCallback(
    (findingId: string) => {
      clearPreview();
      review.selectFinding(findingId);
    },
    [clearPreview, review.selectFinding],
  );
    ...
    <>
      ...
      <div className="col-span-12 grid grid-cols-subgrid gap-5 xl:col-span-9">
        ...
        <DocumentPane
          ...
          onSelectClause={handleSelectClauseFromDocument}
        ...
        <FindingsRail
          ...
          onSelectFinding={handleSelectFindingFromRail}
